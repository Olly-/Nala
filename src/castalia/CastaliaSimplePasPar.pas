...
begin
  if (Lexer.TokenID <> tokString) and (Lexer.TokenID <> tokIdentifier) then
  begin
    if Assigned(FOnMessage) then
      FOnMessage(Self, meError, Format(rsExpected, [TokenName(tokString), fLexer.Token]), fLexer.PosXY.X, fLexer.PosXY.Y);
  end else
    NextToken;
...
begin
  Expected(tokType);
  {$IFDEF D8_NEWER}
  while ((TokenID in [tokIdentifier] + DataTypes) and (Lexer.ExID in ExTypes + DataTypes)) or
        (Lexer.TokenID = tokSquareOpen) do
  begin
    if TokenID = tokSquareOpen then
      CustomAttribute
    else
