...
procedure Lape_TPoint_Inside(Params: PParamArray);
begin
  PPoint(Params^[0])^.Inside(PBox(Params^[1])^);
end;

procedure Lape_TPoint_InsideEx(Params: PParamArray);
begin
...
begin
  ...
  begin
    ...
    addGlobalFunc('function TPoint.Inside(Box: TBox): Boolean;', 'Returns true if the point is located inside the bounds of ''Box''', @Lape_TPoint_Inside);
    addGlobalFunc('function TPoint.Inside(Middle: TPoint; Radius: Int32): Boolean; overload;', 'Returns true if the point is inside the circle defined by ''Middle'' and ''Radius''', @Lape_TPoint_InsideEx);
    addGlobalFunc('function TPoint.Random(Min, Max: Int64): TPoint; ', 'Adds a random integer between ''Min''..''Max'' to both X and Y fields', @Lape_TPoint_Random);
    addGlobalFunc('function TPoint.DistanceTo(Point: TPoint): Double;', 'Returns the rounded distance to ''Point''', @Lape_TPoint_DistanceTo);
